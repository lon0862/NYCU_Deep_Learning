 ...
 * 由index 15倒數至index 0, ex: raw value 4對應2^4=16
 * +------------------------+
 * |     2     8   128     4|
 * |     8    32    64   256|
 * |     2     4    32   128|
...
public:
	pattern(const std::vector<int>& p, int iso = 8) : feature(1 << (p.size() * 4)), iso_last(iso) {
		 ...
		 */
		 // 8個isomorphic為feature轉4次90度, 以及各自的鏡射
		for (int i = 0; i < 8; i++) {
			board idx = 0xfedcba9876543210ull;
			if (i >= 4) idx.mirror();
			idx.rotate(i);
			// 依照feature內的index去旋轉或鏡射的board中取次方的4bit值,並存進isomorphic中
			for (int t : p) {
...
public:
	...
	/**
	 * update the value of a given board, and return its updated value
	 * 因為有4個feature,每個feature有8個isomophric,因此總更新值要被切成4*8份,
	 * 前面已切過4份, 這裡再切成8份
	 */
...
public:
	 ...
	 * 有8個isomorphic,每個有6個feature的index在board中代表的次方值
	 */
...
protected:
	/* feature中有6個index可以在board中取得其各自位置的次方值,
	 * indexof要回傳一個順序為index 0~5的次方值組成的數字,
	 * 因為每個次方值為4bit組成,所以使用4*i去存放在下一個位置
	 */
...
void save_scorecsv(std::string file_name, int mean_score){
    file.open(file_name, std::ios::app);
    file << mean_score << std::endl;
    file.close();
}
...
public:
	 ...
	 * 因為有4個feature,每個feature有8個isomophric,因此總更新值要被切成4*8份,這裡先切成4份
	 */
	...
	/* 先pop_back一次, 從terminal state的前一個state開始更新回去, 
	 * after_state()是滑動完的board, before_state是popup完尚未滑動的board
	 * reward() is the reward of performing action()
	 * value() is the estimated value of after_state(), include all possible popup
	 */
	 ...
	 */
	void make_statistic(std::string file_name, size_t n, const board& b, int score, int unit = 1000) {
		...
		if (n % unit == 0) { // show the training process
			...
			save_scorecsv(file_name, mean); // 1000個存一次mean
...
int main(int argc, const char* argv[]) {
	...
	*/
	...
	for (size_t n = 1; n <= total; n++) {
		...
		b.init(); // 一開始會有2個隨機的2 or 4在board中
		while (true) {
			debug << "state" << std::endl << b;
			state best = tdl.select_best_move(b); // b 是popup完的狀態
			path.push_back(best); //儲存目前board狀態下想要做的移動state

			if (best.is_valid()) {
				debug << "best " << best;
				score += best.reward();
				b = best.after_state(); // b 是滑動完的狀態
				b.popup();
			} else {
				break;
			}
		...
		tdl.make_statistic(csv_name, n, b, score);
		path.clear();

	}

	// store the model into file
	tdl.save("wts/150K.bin");
