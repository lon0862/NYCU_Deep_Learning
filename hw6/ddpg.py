...
class DDPG:
    ...
    def select_action(self, state, noise=True):
        ...
        ## TODO ##
        if noise:
            gaussioan_noise = self._action_noise.sample()        
        else:
            gaussioan_noise = np.zeros(self._action_noise.sample().shape)

        gaussioan_noise = torch.tensor(gaussioan_noise).to(self.device)
        action = self._actor_net(torch.tensor(state).to(self.device)) + gaussioan_noise
...
def main():
    ...
    parser.add_argument('-m', '--model', default='ckpt/ddpg.pth')
